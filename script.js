...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNqaHdqamF5YnZ2eXVydnJ1b3R0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk5ODQ4NjgsImV4cCI6MjA5NTU2MDg2OH0.cIf5waKhivkYpLJ5--apMwK9WHSyum1GH6iKjLfaWbs'; // ← REPLACE THIS
...
const AgentManager = {
    ...
    async create(agentData) {
        ...
        const saved = await DataStore.saveAgent(newAgent);
        if (!saved) return { success: false, message: 'Failed to create agent' };
    ...
    async update(id, updates) {
        ...
        const newUsername = (updates.username || this._cache[index].username).toLowerCase();
        if (newUsername !== this._cache[index].username &&
        ...
        const payload = {
            ...
            display_name: updates.displayName?.trim() || this._cache[index].display_name,
            password: updates.password || this._cache[index].password,
            role: updates.role || this._cache[index].role
        };

        const saved = await DataStore.saveAgent(payload);
        if (!saved) return { success: false, message: 'Failed to update agent' };

        this._cache[index] = saved;
        return { success: true, agent: saved };
    },
    ...
    async delete(id) {
        ...
        if (agent.username === 'admin') return { success: false, message: 'Cannot delete the primary administrator' };
        if (agent.role === 'admin') {
            const adminCount = this._cache.filter(a => a.role === 'admin').length;
            if (adminCount <= 1) return { success: false, message: 'Cannot delete the last administrator' };
        }

        const ok = await DataStore.deleteAgent(id);
        if (!ok) return { success: false, message: 'Failed to delete agent' };
...
const LeadManager = {
    ...
    async save(leadData) {
        ...
        if (existingIndex >= 0) {
            ...
            const saved = await DataStore.saveLead(updated);
            if (saved) this._cache[existingIndex] = saved;
            return { isNew: false, lead: saved || updated, isMaxedOut: newCount >= 3 };
        } else {
            const newLead = {
            ...
            const saved = await DataStore.saveLead(newLead);
            if (saved) this._cache.unshift(saved);
            return { isNew: true, lead: saved || newLead, isMaxedOut: false };
        }
    },

    async delete(id) {
        const ok = await DataStore.deleteLead(id);
        if (ok) this._cache = this._cache.filter(l => l.id !== id);
        return ok;
    },
...
const UI = {
    ...
    bindEvents() {
        ...
        document.querySelectorAll('.nav-link').forEach(link => {
            link.addEventListener('click', (e) => { e.preventDefault(); this.navigateTo(link.dataset.page); document.getElementById('sidebar').classList.remove('open'); });
        });
        ...
        document.getElementById('clearAllBtn').addEventListener('click', () => {
            this.showConfirm('Clear All Data', 'This will permanently delete all leads from the cloud database.', async () => {
                const leads = LeadManager.getAll();
                for (const lead of leads) await DataStore.deleteLead(lead.id);
                await LeadManager.loadAll();
                this.refreshAll();
    ...
    // --- NOTE: All render methods below read from cache (sync) ---
    // The cache is kept fresh by real-time sync + explicit loadAll() calls
    ...
    renderDashboard() {
        ...
        todayEl.innerHTML = todayLeads.length === 0
            ...
            : todayLeads.map(l => `<div class="today-item"><div><div class="today-phone">${this.escapeHtml(l.phone)}</div><div class="today-type">${this.escapeHtml(l.lead_type)} · Follow-up ${l.follow_up_count}/3</div></div><button class="btn btn-primary btn-xs" onclick="UI.openLeadModal('${l.id}')">Update</button></div>`).join('');
    ...
    renderLeadItem(l) {
        ...
        return `<div class="lead-item ${done?'completed':''}"><div class="lead-item-info"><div class="lead-item-header"><span class="lead-phone">${this.escapeHtml(l.phone)}</span><span class="lead-type-badge ${this.getTypeBadgeClass(l.lead_type)}">${this.escapeHtml(l.lead_type)}</span><span class="follow-up-counter ${l.follow_up_count>=3?'max':''}">${l.follow_up_count}/3</span></div><div class="lead-meta"><span class="lead-meta-item"><span class="status-dot ${this.getStatusDotClass(l.call_status)}"></span> ${this.escapeHtml(l.call_status)}</span><span class="lead-meta-item">Agent: ${this.escapeHtml(l.assigned_agent||'Unassigned')}</span>${l.next_follow_up_date?`<span class="lead-meta-item">Follow-up: ${l.next_follow_up_date}</span>`:''}<span class="lead-meta-item">${this.timeAgo(l.last_updated)}</span></div>${l.remarks?`<div class="lead-remarks">"${this.escapeHtml(l.remarks)}"</div>`:''}</div><div class="lead-actions">${!done?`<button class="btn btn-primary btn-xs" onclick="UI.openLeadModal('${l.id}')">Update</button>`:''}${Auth.isAdmin()?`<button class="btn btn-danger btn-xs" onclick="UI.deleteLead('${l.id}')">Delete</button>`:''}</div></div>`;
    },

    renderFollowUps(tab) {
        let leads, msg;
        switch(tab) {
            case 'today': leads=LeadManager.getTodayFollowUps(); msg='No follow-ups today.'; break;
            case 'upcoming': leads=LeadManager.getUpcomingFollowUps(); msg='No upcoming follow-ups.'; break;
            case 'overdue': leads=LeadManager.getOverdueFollowUps(); msg='No overdue follow-ups.'; break;
            default: leads=[];
        }
        const c = document.getElementById('followupList');
        c.innerHTML = leads.length === 0
            ? `<div class="empty-state"><p>${msg}</p></div>`
            : leads.map(l => `<div class="lead-item"><div class="lead-item-info"><div class="lead-item-header"><span class="lead-phone">${this.escapeHtml(l.phone)}</span><span class="lead-type-badge ${this.getTypeBadgeClass(l.lead_type)}">${this.escapeHtml(l.lead_type)}</span><span class="follow-up-counter">${l.follow_up_count}/3</span></div><div class="lead-meta"><span class="lead-meta-item"><span class="status-dot ${this.getStatusDotClass(l.call_status)}"></span> ${this.escapeHtml(l.call_status)}</span><span class="lead-meta-item">Agent: ${this.escapeHtml(l.assigned_agent||'Unassigned')}</span><span class="lead-meta-item">Due: ${l.next_follow_up_date}</span></div></div><div class="lead-actions"><button class="btn btn-primary btn-xs" onclick="UI.openLeadModal('${l.id}')">Update</button></div></div>`).join('');
    },
    ...
    renderAnalytics() {
        ...
        const cc = {'Answer Rate':'var(--status-green)','Completion Rate':'var(--accent-blue)','Decline Rate':'var(--status-red)','No Answer Rate':'var(--status-orange)'};
    ...
    openAgentModal(agentId=null) {
        ...
        if (agentId) {
            ...
            if (a) {
                title.textContent='Edit Agent'; sb.textContent='Update Agent';
            ...
            title.textContent='Add New Agent'; sb.textContent='Save Agent';
    ...
    async handleAgentSubmit() {
        ...
        if (result.success) {
            ...
            if (this.editingAgentId && cu && cu.agentId===this.editingAgentId) {
                cu.displayName=dn; cu.role=ro; DataStore.saveSession(cu);
                this.updateUserInfo(cu); this.toggleAdminFeatures(ro==='admin');
            }
            this.renderAgents(); this.populateLoginDropdown(); this.populateAgentFilter(); this.populateLeadAgentDropdown();
    ...
    deleteAgent(id) {
        ...
        this.showConfirm('Delete Agent',`Delete "${a.display_name}" (@${a.username})?`, async () => {
            ...
            if (r.success) { this.showToast('Agent deleted','success'); this.renderAgents(); this.populateLoginDropdown(); this.populateAgentFilter(); this.populateLeadAgentDropdown(); }
    ...
    openLeadModal(leadId=null) {
        ...
        if (leadId) {
            ...
            if (l) {
                title.textContent='Update Lead'; sb.textContent='Update Lead';
            ...
            title.textContent='Add New Lead'; sb.textContent='Save Lead';
            pi.readOnly=false; fc.value='1 / 3';
    ...
    async handleLeadSubmit() {
        ...
        if (!phone||!lt||!cs||!aa) { this.showToast('Fill in all required fields','error'); return; }
        const result = await LeadManager.save({phone,leadType:lt,callStatus:cs,assignedAgent:aa,nextFollowUpDate:fd,remarks:rm});
        if (result.isMaxedOut) this.showToast('Lead completed — max 3 follow-ups','success');
        else if (result.isNew) this.showToast('New lead created','success');
        else this.showToast(`Lead updated — follow-up ${result.lead.follow_up_count}/3`,'success');
    ...
    checkDuplicate(phone) {
        ...
        if (phone.length>5) {
            ...
            if (ex) { dm.classList.remove('hidden'); dm.textContent=`Existing lead (${ex.follow_up_count}/3) — will update`; }
    ...
    populateLeadAgentDropdown() {
        ...
        AgentManager.getAll().forEach(a=>{const o=document.createElement('option');o.value=a.display_name;o.textContent=a.display_name;s.appendChild(o);});
    },
    ...
    showToast(msg, type='success') {
        ...
        t.className=`toast ${type}`; t.classList.remove('hidden');
    ...
    getStatusDotClass(s) { return {'Answered':'status-answered','Declined':'status-declined','No Answer':'status-noanswer','Busy':'status-busy','Call Back Later':'status-callback'}[s]||'status-answered'; },
